...
function updateReports() {
    ...
    fetch((window.BASE_URL || '/') + `admin/appointments/get_all_appointments?timeRange=${timeRange}`)
        .then(response => {
            if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const exportCounselorFilter = document.getElementById('exportCounselorFilter');
    ...
    async function fetchAppointments() {
        try {
            showLoading();
            const response = await fetch((window.BASE_URL || '/') + 'admin/appointments/get_all_appointments', {
                method: 'GET',
                credentials: 'include',
                headers: {
        ...
        fetch('../admin/filter-data/counselors')
            ...
            .then(data => {
                if (data.success && exportCounselorFilter) {
                    exportCounselorFilter.innerHTML = '<option value="">All Counselors</option>';
                    ...
                    data.data.forEach(counselor => {
                        ...
                        window.__counselorIdToName[String(counselor.counselor_id)] = counselor.name;
        ...
        fetch('../admin/filter-data/courses')
            ...
            .then(data => {
                if (data.success && exportCourseFilter) {
                    exportCourseFilter.innerHTML = '<option value="">All Courses</option>';
        ...
        fetch('../admin/filter-data/year-levels')
            ...
            .then(data => {
                if (data.success && exportYearLevelFilter) {
                    exportYearLevelFilter.innerHTML = '<option value="">All Year Levels</option>';
    ...
    async function applyFilters() {
        ...
        const counselorId = exportCounselorFilter ? exportCounselorFilter.value : '';
    ...
    function applyEnhancedFilters(appointments, filters, reportTitle) {
        ...
        if (filters.startDate || filters.endDate) {
            ...
            if (filters.startDate && filters.endDate) {
                title += ` (${formatDateForTitle(filters.startDate)} - ${formatDateForTitle(filters.endDate)})`;
        ...
        if (filters.counselorId) {
            filteredAppointments = filteredAppointments.filter(app => {
                ...
                if (typeof app.counselor_id !== 'undefined' && app.counselor_id !== null) {
                    if (String(app.counselor_id) === String(filters.counselorId)) return true;
                }
                ...
                if (selectedName && app.counselor_name) {
                    return String(app.counselor_name).trim().toLowerCase() === String(selectedName).trim().toLowerCase();
                }
                return false;
            });
        }
    ...
    async function exportToPDF(filters = {}) {
        try {
            ...
            const tableHeaders = ['User ID', 'Full Name', 'Date', 'Time', 'Type', 'Counselor', 'Status', 'Reason for Status'];
            ...
            // Footer: list filter summary for final page as well
            try {
    ...
    function exportToExcel(filters = {}) {
        ...
        const headerRow = ['User ID', 'Full Name', 'Date', 'Time', 'Consultation Type', 'Counselor', 'Status', 'Reason for Status'];
        ...
        // Set title merge (A1:H1)
        worksheet['!merges'] = [
            { s: { r: 0, c: 0 }, e: { r: 0, c: 7 } }
        ];

        // Footer summary row
        const footerSummary = buildFilterSummary(filters) || 'No additional filters applied';
        excelData.push([]);
        excelData.push([footerSummary]);
