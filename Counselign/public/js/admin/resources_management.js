...
function loadResources() {
    ...
    const url = (window.BASE_URL || '/') + 'admin/resources/get' + (filters ? '?' + filters : '');
...
function renderResources() {
    ...
    container.innerHTML = resources.map(resource => `
        <div class="resource-card" data-resource-id="${resource.id}">
            <div class="d-flex">
                <div class="resource-icon ${resource.resource_type === 'file' ? 'file-icon' : 'link-icon'}">
                    <i class="fas fa-${resource.resource_type === 'file' ? 'file-alt' : 'link'}"></i>
                </div>
                <div class="flex-grow-1">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            <h5 class="mb-1">${escapeHtml(resource.title)}</h5>
                            ${resource.description ? `<p class="text-muted mb-2">${escapeHtml(resource.description)}</p>` : ''}
                        </div>
                        <div class="text-end">
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-2">
                        ...
                        ${resource.visibility !== 'all' ? `<span class="badge bg-info">${formatVisibility(resource.visibility)}</span>` : ''}
                        ${resource.resource_type === 'file' && resource.file_size_formatted ? 
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <small class="text-muted">
                            <i class="fas fa-user me-1"></i>${escapeHtml(resource.uploader_name || 'Unknown')}
                            <i class="fas fa-calendar ms-3 me-1"></i>${resource.created_at_formatted || 'N/A'}
                            ${resource.view_count ? `<i class="fas fa-eye ms-3 me-1"></i>${resource.view_count} views` : ''}
                            ${resource.download_count ? `<i class="fas fa-download ms-3 me-1"></i>${resource.download_count} downloads` : ''}
                        </small>
                        <div>
                            ${resource.resource_type === 'link' ? 
                                `<button class="btn btn-sm btn-info btn-action" onclick="openLink('${escapeHtml(resource.external_url || '')}')" title="Open Link">
                                    <i class="fas fa-external-link-alt"></i>
                                </button>` :
                                `<button class="btn btn-sm btn-primary btn-action" onclick="previewResource(${resource.id})" title="Preview">
                            ...
                            <button class="btn btn-sm btn-warning btn-action" onclick="editResource(${resource.id})" title="Edit">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-danger btn-action" onclick="confirmDeleteResource(${resource.id})" title="Delete">
...
function editResource(id) {
    ...
    if (!resources || resources.length === 0) {
        openAlertModal('Resources not loaded. Please refresh the page and try again.', 'error');
        return;
    }
    ...
    if (!resource) {
        ...
        openAlertModal('Unable to locate the selected resource. Please refresh and try again.', 'error');
        return;
    }
    ...
    if (resource.resource_type === 'link') {
        ...
        if (resource.file_name) {
            ...
            document.getElementById('currentFileName').textContent = resource.file_name;
        }
    }
...
function saveResource() {
    ...
    console.log('Saving resource to:', url);
    console.log('Is Edit Mode:', !!resourceId);
    ...
    .then(data => {
        ...
        if (data.success) {
            openAlertModal(data.message || 'Resource saved successfully', 'success');
            ...
            openAlertModal(data.message || 'Failed to save resource', 'error', { keepActiveModals: true });
        }
    })
    .catch(error => {
        ...
        saveBtn.disabled = false;
        saveBtn.innerHTML = originalText;
        openAlertModal('An error occurred while saving: ' + error.message, 'error', { keepActiveModals: true });
    });
}
        ...
        (context = {}) => {
            ...
            deleteResourceRequest(id)
                .then((data) => {
                    ...
                    openAlertModal(data.message || 'Resource deleted successfully', 'success');
                    loadResources();
                })
                .catch((error) => {
                    if (typeof close === 'function') {
                ...
                .finally(() => {
                    if (typeof reset === 'function') {
                        ...
                        confirmButton.innerHTML = confirmButton.dataset.originalText || '<i class="fas fa-trash me-2"></i>Delete';
...
function downloadResource(id) {
    window.location.href = (window.BASE_URL || '/') + 'admin/resources/download/' + id;
}
...
function previewResource(id) {
    ...
    if (fileType.includes('image')) {
        modalBody.innerHTML = `<img src="${fileUrl}" class="img-fluid" alt="${escapeHtml(resource.title)}">`;
    } else if (fileType.includes('pdf')) {
        modalBody.innerHTML = `<iframe src="${fileUrl}" width="100%" height="600px" style="border:none;"></iframe>`;
        ...
        modalBody.innerHTML = `
            <div class="text-center py-5">
                ...
                <p class="text-muted">${resource.file_name}</p>
