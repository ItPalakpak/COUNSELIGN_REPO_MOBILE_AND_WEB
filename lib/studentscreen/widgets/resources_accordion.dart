...
class ResourcesAccordion extends StatefulWidget {
  const ResourcesAccordion({super.key});

  @override
  State<ResourcesAccordion> createState() => _ResourcesAccordionState();
}

class _ResourcesAccordionState extends State<ResourcesAccordion> {
  ...
  Future<void> _loadResources() async {
    try {
      debugPrint('ResourcesAccordion: Starting to load resources');
      final resources = await ResourceService.fetchResources();
      debugPrint('ResourcesAccordion: Received ${resources.length} resources');
      ...
      if (mounted) {
        ...
        if (_resources.isEmpty) {
          debugPrint('ResourcesAccordion: No resources loaded');
        } else {
          debugPrint('ResourcesAccordion: Successfully loaded ${_resources.length} resources');
          for (var i = 0; i < _resources.length && i < 3; i++) {
            debugPrint('ResourcesAccordion: Resource $i: ${_resources[i].title}');
          }
        }
      }
    } catch (e, stackTrace) {
      debugPrint('ResourcesAccordion: Error loading resources: $e');
      debugPrint('ResourcesAccordion: Stack trace: $stackTrace');
      if (mounted) {
  ...
  void _showPreview(Resource resource) {
    final fileUrl = ResourceService.getFileUrl(resource.filePath);
  ...
  Future<void> _downloadAndOpenFile(Resource resource) async {
    try {
      ...
      final downloadUrl = ResourceService.getDownloadUrl(resource.id);
